    ...
    public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient
    {
        ...
        private class ActiveState : NativeRealtimeMultiplayerClient.MessagingEnabledState
        {
            ...
            internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
            {
                ...
                Logger.d("Updated participant statuses: " + string.Join(",", Enumerable.Select<Participant, string>(base.mParticipants.Values, <>f__am$cache3).ToArray<string>()));
                ...
                list = Enumerable.Where<string>(list, new Func<string, bool>(storey.<>m__0)).ToList<string>();
                list2 = Enumerable.Where<string>(list2, new Func<string, bool>(storey.<>m__1)).ToList<string>();
                if (list.Count > 0)
                {
                    list.Sort();
                    base.mSession.OnGameThreadListener().PeersConnected(Enumerable.Where<string>(list, new Func<string, bool>(storey.<>m__2)).ToArray<string>());
                }
                if (list2.Count > 0)
                {
                    list2.Sort();
                    base.mSession.OnGameThreadListener().PeersDisconnected(Enumerable.Where<string>(list2, new Func<string, bool>(storey.<>m__3)).ToArray<string>());
                }
            }
        ...
        private abstract class MessagingEnabledState : NativeRealtimeMultiplayerClient.State
        {
            ...
            internal sealed override List<Participant> GetConnectedParticipants()
            {
                ...
                List<Participant> list = Enumerable.Where<Participant>(this.mParticipants.Values, <>f__am$cache3).ToList<Participant>();
                list.Sort();
                return list;
            }
            ...
            internal void UpdateCurrentRoom(NativeRealTimeRoom room)
            {
                ...
                this.mNativeParticipants = Enumerable.ToDictionary<GooglePlayGames.Native.PInvoke.MultiplayerParticipant, string>(this.mRoom.Participants(), <>f__am$cache0);
                if (<>f__am$cache1 == null)
                {
                    <>f__am$cache1 = p => p.AsParticipant();
                }
                ...
                this.mParticipants = Enumerable.ToDictionary<Participant, string>(Enumerable.Select<GooglePlayGames.Native.PInvoke.MultiplayerParticipant, Participant>(this.mNativeParticipants.Values, <>f__am$cache1), <>f__am$cache2);
            }
        }
