namespace GooglePlayGames.Native
{
    ...
    internal class NativeSavedGameClient : ISavedGameClient
    {
        ...
        private static SavedGameRequestStatus AsRequestStatus(GooglePlayGames.Native.Cwrapper.CommonErrorStatus.SnapshotOpenStatus status)
        {
            ...
            if (status == GooglePlayGames.Native.Cwrapper.CommonErrorStatus.SnapshotOpenStatus.VALID)
            {
                return SavedGameRequestStatus.Success;
            }
        ...
        public void CommitUpdate(ISavedGameMetadata metadata, SavedGameMetadataUpdate updateForMetadata, byte[] updatedBinaryData, Action<SavedGameRequestStatus, ISavedGameMetadata> callback)
        {
            ...
            {
                this.mSnapshotManager.Commit(metadata2, AsMetadataChange(updateForMetadata), updatedBinaryData, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.CommitResponse>(storey.<>m__0));
            }
        }
        ...
        public void FetchAllSavedGames(GooglePlayGames.BasicApi.DataSource source, Action<SavedGameRequestStatus, List<ISavedGameMetadata>> callback)
        {
            ...
            this.mSnapshotManager.FetchAll(AsDataSource(source), new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.FetchAllResponse>(storey.<>m__0));
        }

        private void InternalOpen(string filename, GooglePlayGames.BasicApi.DataSource source, ConflictResolutionStrategy resolutionStrategy, bool prefetchDataOnConflict, ConflictCallback conflictCallback, Action<SavedGameRequestStatus, ISavedGameMetadata> completedCallback)
        {
            ...
            this.mSnapshotManager.Open(storey.filename, AsDataSource(storey.source), mANUAL, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse>(storey.<>m__0));
        }
        ...
        public void OpenWithAutomaticConflictResolution(string filename, GooglePlayGames.BasicApi.DataSource source, ConflictResolutionStrategy resolutionStrategy, Action<SavedGameRequestStatus, ISavedGameMetadata> completedCallback)
        {
            this.OpenWithAutomaticConflictResolution(filename, source, resolutionStrategy, false, null, completedCallback);
        }

        public void OpenWithAutomaticConflictResolution(string filename, GooglePlayGames.BasicApi.DataSource source, ConflictResolutionStrategy resolutionStrategy, bool prefetchDataOnConflict, ConflictCallback conflictCallback, Action<SavedGameRequestStatus, ISavedGameMetadata> completedCallback)
        {
        ...
        public void ReadBinaryData(ISavedGameMetadata metadata, Action<SavedGameRequestStatus, byte[]> completedCallback)
        {
            ...
            {
                this.mSnapshotManager.Read(metadata2, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.ReadResponse>(storey.<>m__0));
            }
        }
        ...
        public void ShowSelectSavedGameUI(string uiTitle, uint maxDisplayedSavedGames, bool showCreateSaveUI, bool showDeleteSaveUI, Action<SelectUIStatus, ISavedGameMetadata> callback)
        {
            ...
            {
                this.mSnapshotManager.SnapshotSelectUI(showCreateSaveUI, showDeleteSaveUI, maxDisplayedSavedGames, uiTitle, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.SnapshotSelectUIResponse>(storey.<>m__0));
            }
        }
            ...
            internal void <>m__0(GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response)
            {
                ...
                else if (response.ResponseStatus() == GooglePlayGames.Native.Cwrapper.CommonErrorStatus.SnapshotOpenStatus.VALID_WITH_CONFLICT)
                {
                    ...
                    else
                    {
                        ...
                        this.$this.mSnapshotManager.Read(storey.original, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.ReadResponse>(prefetcher.OnOriginalDataRead));
                        this.$this.mSnapshotManager.Read(storey.unmerged, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.ReadResponse>(prefetcher.OnUnmergedDataRead));
                    }
                }
                else
                {
        ...
        private class NativeConflictResolver : IConflictResolver
        {
            ...
            public void ChooseMetadata(ISavedGameMetadata chosenMetadata)
            {
                ...
                {
                    this.mManager.Resolve(metadata, new NativeSnapshotMetadataChange.Builder().Build(), this.mConflictId, delegate (GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response) {
                        if (!response.RequestSucceeded())
                        {
            ...
            public void ResolveConflict(ISavedGameMetadata chosenMetadata, SavedGameMetadataUpdate metadataUpdate, byte[] updatedData)
            {
                ...
                else
                {
                    ...
                    this.mManager.Resolve(metadata, metadataChange, this.mConflictId, updatedData, delegate (GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response) {
                        if (!response.RequestSucceeded())
                        {
                            this.mCompleteCallback(NativeSavedGameClient.AsRequestStatus(response.ResponseStatus()), null);
                        }
