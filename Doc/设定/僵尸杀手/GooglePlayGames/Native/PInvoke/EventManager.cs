namespace GooglePlayGames.Native.PInvoke
{
    ...
    internal class EventManager
    {
        ...
        internal void Fetch(Types.DataSource source, string eventId, Action<FetchResponse> callback)
        {
            ...
            {
                <>f__mg$cache3 = new GooglePlayGames.Native.Cwrapper.EventManager.FetchCallback(GooglePlayGames.Native.PInvoke.EventManager.InternalFetchCallback);
            }
            if (<>f__mg$cache2 == null)
            {
                <>f__mg$cache2 = new Func<IntPtr, FetchResponse>(FetchResponse.FromPointer);
            }
            GooglePlayGames.Native.Cwrapper.EventManager.EventManager_Fetch(this.mServices.AsHandle(), source, eventId, <>f__mg$cache3, Callbacks.ToIntPtr<FetchResponse>(callback, <>f__mg$cache2));
        }
        ...
        internal void FetchAll(Types.DataSource source, Action<FetchAllResponse> callback)
        {
            ...
            {
                <>f__mg$cache1 = new GooglePlayGames.Native.Cwrapper.EventManager.FetchAllCallback(GooglePlayGames.Native.PInvoke.EventManager.InternalFetchAllCallback);
            }
            if (<>f__mg$cache0 == null)
            {
                <>f__mg$cache0 = new Func<IntPtr, FetchAllResponse>(FetchAllResponse.FromPointer);
            }
            GooglePlayGames.Native.Cwrapper.EventManager.EventManager_FetchAll(this.mServices.AsHandle(), source, <>f__mg$cache1, Callbacks.ToIntPtr<FetchAllResponse>(callback, <>f__mg$cache0));
        }
        ...
        internal class FetchAllResponse : BaseReferenceHolder
        {
            ...
            internal List<NativeEvent> Data()
            {
                ...
                return Enumerable.Select<IntPtr, NativeEvent>(PInvokeUtilities.OutParamsToArray<IntPtr>((out_arg, out_size) => GooglePlayGames.Native.Cwrapper.EventManager.EventManager_FetchAllResponse_GetData(base.SelfPtr(), out_arg, out_size)), <>f__am$cache0).ToList<NativeEvent>();
            }
