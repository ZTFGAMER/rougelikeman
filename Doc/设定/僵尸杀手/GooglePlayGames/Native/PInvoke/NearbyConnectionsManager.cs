namespace GooglePlayGames.Native.PInvoke
{
    ...
    internal class NearbyConnectionsManager : BaseReferenceHolder
    {
        ...
        internal void StartAdvertising(string name, List<NativeAppIdentifier> appIds, long advertisingDuration, Action<long, NativeStartAdvertisingResult> advertisingCallback, Action<long, NativeConnectionRequest> connectionRequestCallback)
        {
            ...
            NearbyConnections.NearbyConnections_StartAdvertising(base.SelfPtr(), name, Enumerable.Select<NativeAppIdentifier, IntPtr>(appIds, <>f__am$cache0).ToArray<IntPtr>(), new UIntPtr((ulong) appIds.Count), advertisingDuration, <>f__mg$cache2, Callbacks.ToIntPtr<long, NativeStartAdvertisingResult>(advertisingCallback, <>f__mg$cache0), <>f__mg$cache3, Callbacks.ToIntPtr<long, NativeConnectionRequest>(connectionRequestCallback, <>f__mg$cache1));
        }
