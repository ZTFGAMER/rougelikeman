namespace GooglePlayGames.BasicApi.Video
{
    ...
    public class VideoCapabilities
    {
        ...
        public override string ToString()
        {
            ...
            args[3] = string.Join(",", this.mCaptureModesSupported.Select<bool, string>(<>f__am$cache0).ToArray<string>());
            if (<>f__am$cache1 == null)
            {
                <>f__am$cache1 = p => p.ToString();
            }
            args[4] = string.Join(",", this.mQualityLevelsSupported.Select<bool, string>(<>f__am$cache1).ToArray<string>());
