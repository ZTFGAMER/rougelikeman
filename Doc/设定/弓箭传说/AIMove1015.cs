...
public class AIMove1015 : AIMoveBase
{
    ...
    public AIMove1015(EntityBase entity) : base(entity)
    {
        this.flyframe = 20;
        this.g = 3f;
        this.alltime = 0.6f;
    ...
    protected override void OnInitBase()
    {
        ...
        List<Grid.NodeItem> list = GameLogic.Release.Path.FindingPath(base.m_Entity.position, this.target.position);
        if (list.Count > 0)
        {
            Vector3 worldPosition = GameLogic.Release.MapCreatorCtrl.GetWorldPosition(list[0].x, list[0].y);
            this.endx = worldPosition.x;
            this.endz = worldPosition.z;
        }
        else
        {
