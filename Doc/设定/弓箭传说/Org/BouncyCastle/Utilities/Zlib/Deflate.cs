namespace Org.BouncyCastle.Utilities.Zlib
{
    ...
    public sealed class Deflate
    {
        ...
        private static readonly Config[] config_table = new Config[] { new Config(0, 0, 0, 0, 0), new Config(4, 4, 8, 4, 1), new Config(4, 5, 0x10, 8, 1), new Config(4, 6, 0x20, 0x20, 1), new Config(4, 4, 0x10, 0x10, 2), new Config(8, 0x10, 0x20, 0x20, 2), new Config(8, 0x10, 0x80, 0x80, 2), new Config(8, 0x20, 0x80, 0x100, 2), new Config(0x20, 0x80, 0x102, 0x400, 2), new Config(0x20, 0x102, 0x102, 0x1000, 2) };
        private static readonly string[] z_errmsg = new string[] { "need dictionary", "stream end", string.Empty, "file error", "stream error", "data error", "insufficient memory", "buffer error", "incompatible version", string.Empty };
        ...
        private const int HEAP_SIZE = 0x23d;
        ...
        internal short[] dyn_ltree = new short[0x47a];
        ...
        internal int[] heap = new int[0x23d];
        internal int heap_len;
        internal int heap_max;
        internal byte[] depth = new byte[0x23d];
        internal int l_buf;
        ...
        Label_0002:
            ...
            if (((num != 0L) && (((this.strstart - num) & 0xffff) <= (this.w_size - 0x106))) && (this.strategy != 2))
            {
        ...
        internal int deflate_slow(int flush)
        {
            ...
            while (true)
            {
                ...
                this.match_length = 2;
                if (((num != 0) && (this.prev_length < this.max_lazy_match)) && (((this.strstart - num) & 0xffff) <= (this.w_size - 0x106)))
                {
                    if (this.strategy != 2)
                    {
                        this.match_length = this.longest_match(num);
        ...
        Label_001D:
            ...
            if ((this.strstart - this.block_start) < (this.w_size - 0x106))
            {
                goto Label_001D;
            }
            this.flush_block_only(false);
        ...
        internal int deflateInit(ZStream strm, int level) => 
            this.deflateInit(strm, level, 15);

        internal int deflateInit(ZStream strm, int level, int bits) => 
            this.deflateInit2(strm, level, 8, bits, 8, 0);
        ...
        internal int deflateReset(ZStream strm)
        {
            ...
            this.status = (this.noheader == 0) ? 0x2a : 0x71;
            strm.adler = strm._adler.adler32(0L, null, 0, 0);
            this.last_flush = 0;
            this.tr_init();
            this.lm_init();
        ...
        Label_0000:
            ...
            if (this.strm.avail_in != 0)
            {
                ...
                if ((this.lookahead < 0x106) && (this.strm.avail_in != 0))
                {
                    goto Label_0000;
                }
            }
        ...
        internal void scan_tree(short[] tree, int max_code)
        {
            ...
            for (int i = 0; i <= max_code; i++)
            {
                ...
                if ((++num5 >= num6) || (num3 != num4))
                {
                    if (num5 < num7)
                    {
                        this.bl_tree[num3 * 2] = (short) (this.bl_tree[num3 * 2] + ((short) num5));
                    }
                    else if (num3 != 0)
                    {
                        if (num3 != num2)
        ...
        {
            if (this.bi_valid > (0x10 - length))
            {
                this.bi_buf |= (uint) (val << this.bi_valid);
                this.pending_buf[this.pending++] = (byte) this.bi_buf;
                this.pending_buf[this.pending++] = (byte) (this.bi_buf >> 8);
                this.bi_buf = (uint) (val >> (0x10 - this.bi_valid));
                this.bi_valid += length - 0x10;
            }
            else
            {
                this.bi_buf |= (uint) (val << this.bi_valid);
        ...
        internal void set_data_type()
        {
            ...
            this.data_type = (num3 <= (num2 >> 2)) ? ((byte) 1) : ((byte) 0);
        }
