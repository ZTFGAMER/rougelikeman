namespace Org.BouncyCastle.Security
{
    ...
    public sealed class SignerUtilities
    {
        ...
        public static Asn1Encodable GetDefaultX509Parameters(DerObjectIdentifier id) => 
            GetDefaultX509Parameters(id.Id);
        ...
        public static Asn1Encodable GetDefaultX509Parameters(string algorithm)
        {
            ...
            if (Platform.EndsWith(source, "withRSAandMGF1"))
            {
                return GetPssX509Parameters(source.Substring(0, source.Length - "withRSAandMGF1".Length));
            }
        ...
        public static DerObjectIdentifier GetObjectIdentifier(string mechanism)
        {
            ...
            string str = (string) algorithms[mechanism];
            if (str != null)
            {
                mechanism = str;
            }
        ...
        {
            AlgorithmIdentifier parameters = new AlgorithmIdentifier(DigestUtilities.GetObjectIdentifier(digestName), DerNull.Instance);
            AlgorithmIdentifier maskGenAlgorithm = new AlgorithmIdentifier(PkcsObjectIdentifiers.IdMgf1, parameters);
            int digestSize = DigestUtilities.GetDigest(digestName).GetDigestSize();
            return new RsassaPssParameters(parameters, maskGenAlgorithm, new DerInteger(digestSize), new DerInteger(1));
        }
