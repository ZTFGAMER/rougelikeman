namespace Org.BouncyCastle.Crypto.Engines
{
    ...
    public sealed class BlowfishEngine : IBlockCipher
    {
        ...
        private static readonly int ROUNDS = 0x10;
        private const int BLOCK_SIZE = 8;
        private static readonly int SBOX_SK = 0x100;
        private static readonly int P_SZ = (ROUNDS + 2);
        private readonly uint[] S0 = new uint[SBOX_SK];
        private readonly uint[] S1 = new uint[SBOX_SK];
        private readonly uint[] S2 = new uint[SBOX_SK];
        private readonly uint[] S3 = new uint[SBOX_SK];
        private readonly uint[] P = new uint[P_SZ];
        private bool encrypting;
        private byte[] workingKey;

        private void DecryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex)
        {
            uint x = Pack.BE_To_UInt32(src, srcIndex);
            uint num2 = Pack.BE_To_UInt32(src, srcIndex + 4);
            x ^= this.P[ROUNDS + 1];
            for (int i = ROUNDS; i > 0; i -= 2)
            {
                num2 ^= this.F(x) ^ this.P[i];
                x ^= this.F(num2) ^ this.P[i - 1];
            }
            num2 ^= this.P[0];
            Pack.UInt32_To_BE(num2, dst, dstIndex);
            Pack.UInt32_To_BE(x, dst, dstIndex + 4);
        }
        ...
        private void EncryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex)
        {
            ...
            num2 ^= this.P[ROUNDS + 1];
            Pack.UInt32_To_BE(num2, dst, dstIndex);
            Pack.UInt32_To_BE(x, dst, dstIndex + 4);
        }

        private uint F(uint x) => 
            (((this.S0[x >> 0x18] + this.S1[(int) ((IntPtr) ((x >> 0x10) & 0xff))]) ^ this.S2[(int) ((IntPtr) ((x >> 8) & 0xff))]) + this.S3[(int) ((IntPtr) (x & 0xff))]);
        ...
        private void ProcessTable(uint xl, uint xr, uint[] table)
        {
            ...
            for (int i = 0; i < length; i += 2)
            {
                ...
                {
                    xr ^= this.F(xl) ^ this.P[j];
                    xl ^= this.F(xr) ^ this.P[j + 1];
                }
                xr ^= this.P[ROUNDS + 1];
                table[i] = xr;
                table[i + 1] = xl;
        ...
        {
            Array.Copy(KS0, 0, this.S0, 0, SBOX_SK);
            Array.Copy(KS1, 0, this.S1, 0, SBOX_SK);
            Array.Copy(KS2, 0, this.S2, 0, SBOX_SK);
            Array.Copy(KS3, 0, this.S3, 0, SBOX_SK);
            Array.Copy(KP, 0, this.P, 0, P_SZ);
